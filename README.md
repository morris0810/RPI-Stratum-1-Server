...
Turn a Raspberry Pi + GPS hat into a stratum-1 time source using chrony with GPS (NMEA) and PPS. Then point OPNsense and TrueNAS SCALE at it for rock-solid time across your network.
✅ This README is written for Pi OS/Debian-like systems and recent OPNsense/TrueNAS SCALE. Commands assume a Pi on Ethernet.
...
GPS Hat that provides NMEA (serial/USB) and PPS
...
**Network plan**
Reserve a static DHCP lease for the Pi (e.g. 192.168.2.22).
...
_bash
sudo apt update
sudo apt install -y chrony gpsd gpsd-clients pps-tools_
...
sudo ppstest /dev/pps0   # should print timestamps (Ctrl+C to stop)_
...
sudo systemctl enable --now gpsd_
...
sudo systemctl enable --now chrony_
...
-Time servers: 192.168.2.2 (✓ iburst)
...
_bash
restrict 192.168.2.0 mask 255.255.255.0 nomodify notrap nopeer_

(CIDR like 192.168.2.0/24 will trigger “unusable” syntax errors.)
...
MS Name/IP address   Stratum Poll Reach LastRx Last sample
^* 192.168.2.2            1   6   377    100    -2ms[   0ms] +/- 20ms
Packet peek (on TrueNAS):
sudo tcpdump -ni any 'udp port 123 and host 192.168.2.2' -vv -c 6
...
sudo tcpdump -ni any 'udp port 123 and host 192.168.2.2' -vv -c 6
Firewalls
Pi (UFW): sudo ufw allow proto udp from 192.168.2.0/24 to any port 123
...
# Upstream (Pi)
server 192.168.2.2 iburst maxpoll 9
...
# On the Pi:
_sudo apt update && sudo apt install -y chrony gpsd gpsd-clients pps-tools
echo 'dtoverlay=pps-gpio,gpiopin=18' | sudo tee -a /boot/firmware/config.txt
...
sudo ufw allow proto udp from 192.168.2.0/24 to any port 123 || true
chronyc sources -v && chronyc tracking_
